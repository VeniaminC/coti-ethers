...
export function encrypt(
...
): { ciphertext: Uint8Array; r: Uint8Array } {
	...
	const plaintextPadded = new Uint8Array([
		...new Uint8Array(BLOCK_SIZE - plaintext.length),
		...plaintext,
	]);
...
export function generateRSAKeyPair(): {
	...
	const privateKey = forge.asn1.toDer(
		forge.pki.privateKeyToAsn1(rsaKeyPair.privateKey),
	).data;
	const publicKey = forge.asn1.toDer(
		forge.pki.publicKeyToAsn1(rsaKeyPair.publicKey),
	).data;
...
export function decryptRSA(privateKey: Uint8Array, ciphertext: string): string {
	...
	const privateKeyPEM = forge.pki.privateKeyToPem(
		forge.pki.privateKeyFromAsn1(
			forge.asn1.fromDer(forge.util.createBuffer(privateKey)),
		),
	);
...
export function sign(message: string, privateKey: string) {
	...
	const sig = key.signDigest(arrayify(message));
	return new Uint8Array([
		...arrayify(sig.r),
		...arrayify(sig.s),
		...arrayify(`0x0${sig.v - 27}`),
	]);
}

export function signInputText(
	sender: { wallet: Wallet; userKey: string },
...
export function buildStringInputText(
	...
	for (
		let startIdx = 0;
		startIdx < encodedStr.length;
		startIdx += EIGHT_BYTES
	) {
		const endIdx = Math.min(startIdx + EIGHT_BYTES, encodedStr.length);

		const byteArr = new Uint8Array([
			...encodedStr.slice(startIdx, endIdx),
			...new Uint8Array(EIGHT_BYTES - (endIdx - startIdx)),
		]); // pad the end of the string with zeros if needed
...
export function decryptUint(ciphertext: ctUint, userKey: string): bigint {
	...
	ctArray = new Uint8Array([
		...new Uint8Array(32 - ctArray.length),
		...ctArray,
	]);
...
export function decryptString(ciphertext: ctString, userKey: string): string {
	...
	return decoder.decode(encodedStr).replace(/\0/g, "");
}
...
export function encodeString(str: string): Uint8Array {
	return new Uint8Array([
		...str
			.split("")
			.map((char) =>
				parseInt(char.codePointAt(0)?.toString(HEX_BASE)!, HEX_BASE),
			),
	]);
}
